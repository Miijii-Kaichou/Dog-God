...
namespace UnityEngine
{
    ...
    public class Alarm
    {
        static WaitForEndOfFrame endOfFrameDelay => new WaitForEndOfFrame();
        ...
        {
            get { return registeredTimers[index]; }
            set { registeredTimers[index] = value; }
        }
...
#if UNITY_EDITOR
                    LogCurrentTime(index, timeEntry);
        ...
        private void LogTimesUp(int index)
        {
            if (debugEnabled) Debug.Log("Timer[" + index + "] returned a value of 1!");
        }

        private void LogCurrentTime(int index, Timer timeEntry)
        {
            if (debugEnabled)
            {
        ...
        private void LogTimeStart(int index)
        {
        ...
        public void Start(int index, ref float currentTime)
        {
            Start(index, Mathf.Infinity, false);
        }
        ...
        private void LogTimeStop(int index)
        {
